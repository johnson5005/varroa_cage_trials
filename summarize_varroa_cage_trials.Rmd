...
plot(fen$prop.mites.48 ~ as.factor(fen$pfa))
plot((fen$bee.48/fen$total.bees.count) ~ as.factor(fen$pfa))
anova(glm(asin(fen$prop.mites.48) ~ fen$pfa))


clove <- subset(cage, active=="clove oil")
plot(clove$prop.mites.48 ~ as.factor(clove$pfa))
plot((clove$bee.48/clove$total.bees.count) ~ as.factor(clove$pfa))
